...
const CabBooking = () => {
  ...
  // Fetch customer details from the backend based on logged-in email
  useEffect(() => {
    const fetchCustomerDetails = async () => {
      const email = localStorage.getItem('email');  // Get email from localStorage
      if (email) {
        try {
          const response = await axios.get(`http://localhost:8081/customers/${email}`);
  ...
  useEffect(() => {
    const fetchCarTypes = async () => {
      try {
        const response = await axios.get('http://localhost:8081/admin/cars');
        const uniqueCarTypes = [...new Set(response.data.map((car) => car.vehicleType))];
        setCarTypes(uniqueCarTypes);
      } catch (error) {
        console.error('Error fetching car types:', error);
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:8081/bookings', newBooking);
      setStatus(`${response.data.customerName}, Your booking is successful..!`);
